...
La simulation tourne autour d'une application serveur REST appelé ici "Manager".
...
la section \ref{sec:problemes}.\\
% TODO: Parler de ça dans les problèmes / perspectives (André/Yann)

Nous avons aussi réalisé un diagramme de classes (voir figure \ref{fig:classes}) qui permet de visualiser certains aspects du projet. On peut voir notement
que les drones possèdent des modules qui peuvent être des capteurs ou des actionneurs, si on veut rajouter un capteur à un drone nous n'avons qu'à
...
être utilisé avec n'importe quelle interface de pilotage, nous avons créé une interface ControleurDrone qui doit être implémentée par les controleurs, ainsi si on veut
...
(Gstreamer) et son implémentation en Java (voir \texttt{fr.univtln.infomath.dronsim.server.simulation.client} et\\
